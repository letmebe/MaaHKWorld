...
class FishingMultiMatchRecognition(CustomRecognition):
    ...
    def analyze(self, context: Context, argv) -> Union[CustomRecognition.AnalyzeResult, Optional[RectType]]:
        ...
        if best_single:
            ...
            if result:
                box = result[1]  # (match_x, match_y, tw, th)
            else:
                box = (0, 0, 0, 0)
...
def mark_roi_on_image():
    """
    在fishing.png上标注ROI区域，另存为fishingmarked.png
    """
    import cv2
    from pathlib import Path
    
    # 查找fishing.png路径
    dev_path = Path(__file__).parent.parent / 'assets' / 'resource' / 'image' / 'fishing.png'
    release_path = Path(__file__).parent.parent / 'resource' / 'image' / 'fishing.png'
    
    input_path = dev_path if dev_path.exists() else release_path
    
    if not input_path.exists():
        print(f"未找到fishing.png: {input_path}")
        return
    ...
    # 定义ROI区域和颜色
    rois = [
        ((1620, 800, 280, 260), (0, 0, 255), "roi_fishing"),      # 红色 - 钓鱼区域
        ((190, 130, 1510, 690), (0, 255, 0), "roi_quick"),        # 绿色 - 快速按钮区域
        ((1515, 910, 155, 60), (255, 0, 0), "roi_bag"),           # 蓝色 - 背包区域
        ((1694, 950, 187, 108), (0, 255, 255), "roi_return"),     # 黄色 - 返回区域
    ]
    ...
    # 保存标注后的图片
    output_path = input_path.parent / 'fishingmarked.png'
