...
class IncrementWaterCountIfHit(CustomAction):
    ...
    def run(self, context: Context, argv) -> bool:
        ...
        """ 由于浇水和采摘都是rb键，所以采摘了作物时会被好友的看田宠物送回自己家，
        结果还在自己田里傻傻前进一步点两下rb，由于自家农田和别家农田的退出条件不一样，所以始终无法退出循环。
        只要在再加入个文本条件："需要在已开垦田地上播种"，那么即使在自己农田里也会退出当前好友浇水循环了。 """

        if not texts:
            # 未识别到文本，啥也不做
            log(f"[IncrementWaterCountIfHit] 未识别到文本，保持计数: {controller.water_count}")
        elif any("这里不是可摘取的地方" in t or "当前位置无法操作" in t or "需要在已开垦田地上播种" in t for t in texts):
            # 识别到目标文本，次数加一
