...
class ActivateWeGameWindow(CustomAction):
    """激活WeGame窗口"""
    
    def run(self, context: Context, argv) -> bool:
        log("[WeGame] 激活WeGame窗口...")
        
        if not _WIN32_AVAILABLE:
            log("[WeGame] win32gui不可用")
            return False
        ...
        try:
            ...
            if not hwnd:
                log("[WeGame] 未找到WeGame窗口")
                return False
            
            log(f"[WeGame] 找到WeGame窗口: HWND={hwnd}")
            
            # 激活窗口
            _force_foreground_window(hwnd)
            
            log("[WeGame] WeGame窗口已激活")
            time.sleep(1.0)
            
            return True
            
        except Exception as e:
            log(f"[WeGame] 激活窗口失败: {e}")
            return False


class CheckGameProcess(CustomAction):
    """检测游戏进程"""
    
    def run(self, context: Context, argv) -> bool:
        log("[Process] 检测王者荣耀世界进程...")
        ...
        try:
            ...
            for proc in psutil.process_iter(['name']):
                try:
                    if proc.info['name'] and '王者荣耀世界' in proc.info['name']:
                        log(f"[Process] 找到游戏进程: {proc.info['name']} (PID: {proc.pid})")
                        return True
            ...
            log("[Process] psutil未安装，使用备用方法...")
            
            # 备用方法：查找窗口
            if _WIN32_AVAILABLE:
                hwnd = win32gui.FindWindow("UnrealWindow", "王者荣耀世界")
                if hwnd:
                    log(f"[Process] 找到游戏窗口: HWND={hwnd}")
                    return True
...
class OpenBrowserAndFocus(CustomAction):
    """打开浏览器并置顶MFAAvalonia窗口"""
    
    def run(self, context: Context, argv) -> bool:
        import webbrowser
        
        log("[Browser] 打开浏览器访问GitHub页面...")
        
        # 打开浏览器
        url = "https://github.com/letmebe/MaaHKWorld"
        webbrowser.open(url)
        
        log(f"[Browser] 已打开: {url}")
        time.sleep(2.0)
        
        # 置顶MFAAvalonia窗口
        if _WIN32_AVAILABLE:
            log("[Browser] 置顶MFAAvalonia窗口...")
            
            # 查找MFAAvalonia窗口
            hwnd = win32gui.FindWindow(None, "MFAAvalonia")
            if hwnd:
                _force_foreground_window(hwnd)
                log("[Browser] MFAAvalonia窗口已置顶")
            else:
                log("[Browser] 未找到MFAAvalonia窗口")
