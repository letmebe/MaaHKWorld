...
本程序是为了降低钓鱼等重复性操作的强度，避免错过浇水收割时间，只适合挂机使用，如果自己有空点点点还是要比它快得多。
...
由于每次游戏升级都可能修改游戏内的操作逻辑，所以如果发现任务执行失败，请帮忙提交issue，或者隔个一两天来喵一眼，我测试发现问题会尽快适配更新。

### 2026-07-07 v2.1.4

- 牧场管理部分增加应对家园升级后选项重置（右下角显示鹿头）的逻辑
- 农田种植参数调整，基本不会有漏浇漏种问题了
- 解决为好友浇水时被宠物赶回家造成任务死循环的问题
- 部分文字说明修改
...
5. 为指定好友浇水功能，使用前务必修改选项中的好友列表！默认项是我的角色，请加我好友为我浇水🤝。

6. MFAAvalonia 会自动保存修改后的选项，无须每次执行任务前都操作。但由于此程序更新都是完整覆盖，务必在更新前备份自己的自定义选项如好友列表等，更新后重新设置。

7. 如果有自动定时运行的需要，可以在 MFAAvalonia - 设置 - **定时执行** 中设定时间，届时会自动运行已选任务。建议长时间挂机时再使用这个功能，否则会干扰电脑正常工作————突然窜出来跟你抢鼠标😂。
