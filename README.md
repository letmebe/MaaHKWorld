...
6. 在 MFAAvalonia 中选择任务 "自动钓鱼"，点击 **开始任务** ，会看到任务栏中窗口图标被激活，延时片刻后游戏内角色开始自动钓鱼，支持包括抛竿、拉杆、X/B连点、A/B/X/Y QTE 在内的全部操作类型。
![运行界面](./assets/resource/image/running.png)

7. 钓鱼过程中尽量保持游戏处于后台，不要切换到游戏画面中点击鼠标，否则输入会从虚拟手柄切换回鼠标，且再次切出画面时窗口焦点丢失，造成角色不再动作。如果遇到这种情况，可以点击 MFAAvalonia 的 **停止任务** ，然后点击 **开始任务** 重新开始钓鱼。
